...
class TestDistributedPrimitives:
    @pytest.mark.anyio
    async def test_incr_delegates_to_db(self):
        cache, mock_db = make_cache()
        mock_db.incr.return_value = 5
        assert await cache.incr("counter", 2) == 5
        mock_db.incr.assert_called_once_with("counter", 2)

    @pytest.mark.anyio
    async def test_set_lock_delegates_to_db(self):
        cache, mock_db = make_cache()
        mock_db.set_lock.return_value = True
        assert await cache.set_lock("lock", "owner", "30s") is True
    ...
    @pytest.mark.anyio
    async def test_unlock_delegates_to_db(self):
        cache, mock_db = make_cache()
        mock_db.unlock.return_value = True
        assert await cache.unlock("lock", "owner") is True
        mock_db.unlock.assert_called_once_with("lock", b"owner")

    @pytest.mark.anyio
    async def test_is_locked_polls(self):
        cache, mock_db = make_cache()
        mock_db.is_locked.side_effect = [True, False]
        
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            assert await cache.is_locked("lock", wait=1.0, step=0.1) is False
    ...
    async def test_lock_context_manager(self):
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            async with cache.lock("mylock", "10s"):
                pass
